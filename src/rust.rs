...
impl<T: GCMethods + Copy> Heap<T> {
    ...
    /// This creates a `Box`-wrapped Heap value. Setting a Heap object value
    /// triggers a barrier with its location, hence why it is not  safe to
    /// construct a temporary Heap value, assign a non-null value and move it
